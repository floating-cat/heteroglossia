...
if [[ "$yN" =~ ^(y|yes)$ ]]; then
  ...
  curl -OL# "https://github.com/floating-cat/domain-ip-set-rules/raw/release/domain-ip-set-rules.db"
  echo "Downloading the domain and IP set rules' sha256sum..."
  curl -OL# "https://github.com/floating-cat/domain-ip-set-rules/raw/release/domain-ip-set-rules.db.sha256sum"
