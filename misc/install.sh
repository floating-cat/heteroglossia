...
if [[ -x "$(command -v curl)" ]]; then
  version=$(curl -fsL https://api.github.com/repos/floating-cat/heteroglossia/releases/latest | grep '"tag_name":' | cut -d'"' -f4)
else
  version=$(wget -qO- https://api.github.com/repos/floating-cat/heteroglossia/releases/latest | grep '"tag_name":' | cut -d'"' -f4)
fi

echo "Downloading the latest release version ${version:1} of heteroglossia(hg)..."
download_file_with_progress_bar "https://github.com/floating-cat/heteroglossia/releases/download/$version/heteroglossia_${version:1}_${os}_${arch}.tar.gz"
echo "Downloading the heteroglossia(hg) release's sha256sum..."
download_file_with_progress_bar "https://github.com/floating-cat/heteroglossia/releases/download/$version/sha256sums.txt"
...
if [[ "$yN" =~ ^(y|yes)$ ]]; then
  ...
  download_file_with_progress_bar "https://github.com/floating-cat/domain-ip-set-rules/raw/release/domain-ip-set-rules.db"
  echo "Downloading the domain and IP set rules' sha256sum..."
  download_file_with_progress_bar "https://github.com/floating-cat/domain-ip-set-rules/raw/release/domain-ip-set-rules.db.sha256sum"
