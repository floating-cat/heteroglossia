...
import (
	...
	"github.com/floating-cat/heteroglossia/transport/http"
...
func (s *server) ListenAndServe(ctx context.Context) error {
	...
	if host == "::1" {
		ipv4Localhost := netutil.JoinHostPort("127.0.0.1", s.httpSOCKS.Port)
	...
	addr := netutil.JoinHostPort(host, s.httpSOCKS.Port)
...
func (s *server) Serve(ctx context.Context, conn net.Conn) error {
	...
	switch b {
	case socks.Version4:
		log.Info("routing", contextutil.SourceTag, conn.RemoteAddr().String(),
